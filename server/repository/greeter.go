...
func (repo greeterRepository) GetGreeterById(ctx context.Context, id int32, opt ...GreeterByIdOption) (model.Greeter, error) {
...
func (repo greeterRepository) FindGreetersCount(ctx context.Context, status int32) (int64, error) {
	var count int64
	tx := repo.ReadDB(ctx).Model(model.Greeter{}).Select("count(id)")
...
func (repo greeterRepository) FindGreeterListIds(ctx context.Context, status, lastId, pageSize int32) ([]int32, []*redis.Z, error) {
	...
	tx := repo.ReadDB(ctx).Model(model.Greeter{}).Select("id")
	tx = tx.Where("status=?", status)
	tx = tx.Order("id DESC")
	rows, err := tx.Rows()
	if err != nil {
...
func (repo greeterRepository) GetGreeterList4Concurrent(ctx context.Context, ids []int32, fn func(context.Context, int32, ...GreeterByIdOption) (model.Greeter, error)) ([]model.Greeter, error) {
...
func (repo greeterRepository) UpdateGreeterStatus(ctx context.Context, id, status int32) (int64, error) {
	...
	tx := repo.WriteDB(ctx).Model(model.Greeter{}).Where("id = ?", id)
...
func (repo greeterRepository) DeleteGreeterById(ctx context.Context, id int32) (int64, error) {
	...
	tx := repo.WriteDB(ctx).Delete(&model.Greeter{}, id)
	if tx.Error != nil {
